...
export default function MoviesCard({ movie, savedMovies, setSavedMovies }) {
  ...
  const buttonClasses = location.pathname === '/movies' ? favorite : deleteButton;
  const buttonsFunction = location.pathname === '/movies' ? handleSaveMovie : handleDeleteMovie;
    ...
    <li className="movies-card" id={movie._id}>
      ...
      <div className="movies-card__description">
        ...
        <div className="movies-card__duration-block">
          <p className="movies-card__duration">{getTranslateTime(movie.duration)}</p>
        </div>
      </div>
